...
export function useAPIInfo () {
    ...
    const hostname =  url?.hostname ?? qs.get('host') ?? localStorage?.[0]?.hostname ?? '127.0.0.1'
    const port =  url?.port ?? qs.get('port') ?? localStorage?.[0]?.port ?? '9090'
    const secret =  url?.username ?? qs.get('secret') ?? localStorage?.[0]?.secret ?? ''
